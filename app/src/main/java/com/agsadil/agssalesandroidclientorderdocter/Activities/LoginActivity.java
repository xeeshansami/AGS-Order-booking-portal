...
public class LoginActivity extends AppCompatActivity {
    ...
    // Media permissions were removed: the app never picks photos/videos, and
    // Play rejects READ_MEDIA_IMAGES/VIDEO for apps without persistent media use.
    String[] permissions2 = {
            Manifest.permission.ACCESS_FINE_LOCATION,
            Manifest.permission.ACCESS_COARSE_LOCATION};
...
//        myToolbar.setNavigationIcon(R.drawable.ic_login);
    ...
    // Android 13 (API 33) and above require an explicit runtime permission before
    // the app may display notifications. Requested separately so it never blocks
    // the login/permission gating above.
...
//    }
    ...
    public void online(Button button, boolean isCheckedRemember) {
        ...
        boolean sameUser = sp.getusername() != null && sp.getpassword() != null
                && sp.getusername().equals(username) && sp.getpassword().equals(userpassword);

        // Same user who already has the master data on the device -> flow normally,
        // do NOT download the master data again.
        if (sameUser && isMasterDataDownloaded()) {
            utils.hideLoader();
            sp.setLogin(true);
            ...
            // New user, or master data missing/incomplete -> authenticate online and
            // download the master data into the DB (handled inside utils.Login -> ChangeView).
            utils.loginOrActiveCheck(false, true, false, button, username, userpassword, isCheckedRemember);
        }
    }

    /**
     * Returns true when the local SQLite DB already holds the full master data set
     * (customers, products and salesmen). Used to decide whether a re-download is needed.
     */
    private boolean isMasterDataDownloaded() {
        return db.getAllCustomers().size() > 0
                && db.getAllProducts().size() > 0
    ...
    public void offline(Button button) {
        ...
        if (sameUser && isMasterDataDownloaded()) {
            // Credentials match and master data is already on the device -> log in offline.
            sp.setLogin(true);
            utils.hideLoader();
            button.setEnabled(true);
            button.setClickable(true);
            ...
            // Credentials are valid but the master data was never downloaded on this device.
            utils.hideLoader();
            button.setEnabled(true);
            button.setClickable(true);
            utils.alertBox(LoginActivity.this, "No data available",
                    "You are offline and the master data has not been downloaded yet. Please connect to the internet for your first login.",
                    "ok", new setOnitemClickListner() {
                        @Override
                        public void onClick(DialogInterface view, int i) {
                            view.dismiss();
            ...
            button.setEnabled(true);
            button.setClickable(true);
            utils.alertBox(LoginActivity.this, "Alert", "Username or password is invalid please login again and enable your internet connection", "ok", new setOnitemClickListner() {
                @Override
                public void onClick(DialogInterface view, int i) {
                    view.dismiss();
                }
